  ...
  return (
    <>
      <div className="grid grid-cols-4 items-center gap-4">
        ...
        {id === 'product' ? <ProductCombobox className={errorClass} product={value as string} onChange={onChange} /> : <Input id={id} className={cn("col-span-3",errorClass)} value={value} onChange={(e)=>{ onChange(e.target.id,e.target.value)}}/>}
      </div>
